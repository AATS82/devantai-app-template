...
// ============================================================
// DEVANTAI — LANDING TEMPLATE: PIZZERÍA
// Variantes dinámicas: hero_variant, servicios_variant, nosotros_variant
// Perfil: cálido_familiar con tema oscuro/claro
// ============================================================
...
const DEFAULT_DATA = {
    ...
    imagen_hero: "https://images.unsplash.com/photo-1604068549290-dea0e4a305ca?w=1800&q=80",
    imagen_nosotros: "https://images.unsplash.com/photo-1551632786-de41ec24a8f6?w=800&q=80",
    servicios: [
        { titulo: "{{SERVICIO_1}}", descripcion: "{{DESC_1}}", icon: "pizza" },
        { titulo: "{{SERVICIO_2}}", descripcion: "{{DESC_2}}", icon: "leaf" },
        { titulo: "{{SERVICIO_3}}", descripcion: "{{DESC_3}}", icon: "truck" },
        { titulo: "{{SERVICIO_4}}", descripcion: "{{DESC_4}}", icon: "clock" },
    ],
    testimonios: [
...
// SVG Icons para Pizzería
const Icons = {
    pizza: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L2 20h20Z" /><path d="M12 11v9" /><path d="M8 17h8" /></svg>),
    leaf: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2c-5.5 4-8 7-8 11 0 4 3 7 8 7s8-3 8-7c0-4-2.5-7-8-11z" /></svg>),
    truck: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13" /><polygon points="16 8 20 8 23 11 23 16" /><circle cx="5.5" cy="18.5" r="2.5" /><circle cx="18.5" cy="18.5" r="2.5" /></svg>),
...
function HeroCentered({ d, primary, primaryRgb, fontTitulo, fontCuerpo, scrollTo, BG, BG2, TEXT, MUTED, BORDER, isDark }) {
    return (
        <section id="inicio" style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", background: BG }}>
            <div style={{ position: "absolute", inset: 0, backgroundImage: `url('${d.imagen_hero || 'https://images.unsplash.com/photo-1604068549290-dea0e4a305ca?w=1800&q=80'}')`, backgroundSize: "cover", backgroundPosition: "center", filter: isDark ? "brightness(0.35)" : "brightness(0.8)" }} />
            <div style={{ position: "absolute", inset: 0, background: `linear-gradient(135deg, rgba(${isDark ? '15,23,42' : '255,255,255'},0.85) 0%, rgba(${primaryRgb},0.15) 100%)` }} />
            ...
            <div style={{ position: "relative", textAlign: "center", padding: "0 1.5rem", maxWidth: 780, zIndex: 1 }}>
                ...
                <div style={{ display: "flex", gap: "1rem", justifyContent: "center", flexWrap: "wrap" }}>
                    <button className="btn-primary" style={{ padding: "1rem 2.5rem", fontSize: "1rem", borderRadius: "12px" }} onClick={() => scrollTo("contacto")}>Pedir Ahora</button>
                    <button className="btn-outline" style={{ padding: "1rem 2.5rem", fontSize: "1rem", borderRadius: "12px" }} onClick={() => scrollTo("servicios")}>Ver Menú</button>
                </div>
            </div>
        </section>
    );
        ...
        <section id="inicio" style={{ position: "relative", minHeight: "100vh", display: "grid", gridTemplateColumns: "1fr 1fr", overflow: "hidden" }}>
            <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", padding: "8rem 3rem 4rem 4rem", background: BG, zIndex: 1 }}>
                ...
                <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
                    <button className="btn-primary" style={{ padding: "1rem 2rem", fontSize: "0.95rem", borderRadius: "12px" }} onClick={() => scrollTo("contacto")}>Pedir Ahora</button>
                    <button className="btn-outline" style={{ padding: "1rem 2rem", fontSize: "0.95rem", borderRadius: "12px" }} onClick={() => scrollTo("servicios")}>Ver Menú</button>
                </div>
            </div>
            <div style={{ position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", inset: 0, backgroundImage: `url('${d.imagen_hero || 'https://images.unsplash.com/photo-1604068549290-dea0e4a305ca?w=1800&q=80'}')`, backgroundSize: "cover", backgroundPosition: "center" }} />
...
function ServiciosGrid({ d, primary, primaryRgb, fontTitulo, fontCuerpo, BG, BG2, TEXT, MUTED, BORDER, isDark }) {
    return (
        <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: "1.5rem" }}>
            {(d.servicios || []).map((s, i) => (
                <AnimatedSection key={i} delay={i * 100}>
                    <div className="service-card" style={{ background: BG2, border: `1px solid ${BORDER}`, borderRadius: "20px", overflow: "hidden", transition: "all 0.3s", cursor: "pointer" }}>
                        ...
                        <div style={{ padding: "2rem", display: "flex", gap: "1.5rem", alignItems: "flex-start" }}>
                            <div style={{ width: 52, height: 52, minWidth: 52, background: `rgba(${primaryRgb},0.15)`, border: `1px solid rgba(${primaryRgb},0.3)`, display: "flex", alignItems: "center", justifyContent: "center", color: primary, borderRadius: "12px" }}>
                                <span style={{ width: 24, height: 24 }}>{Icons[s.icon] || Icons.pizza}</span>
                            </div>
                            <div>
...
function ServiciosList({ d, primary, primaryRgb, fontTitulo, fontCuerpo, BG, BG2, TEXT, MUTED, BORDER, isDark }) {
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            {(d.servicios || []).map((s, i) => (
                <AnimatedSection key={i} delay={i * 80}>
                    <div style={{ display: "flex", alignItems: "center", gap: "2rem", padding: "1.75rem 2rem", background: BG2, border: `1px solid ${BORDER}`, borderLeft: `4px solid ${primary}`, transition: "all 0.3s", borderRadius: "16px" }}
                        ...
                        <div style={{ width: 40, height: 40, minWidth: 40, color: primary }}>
                            <span style={{ width: 28, height: 28, display: "block" }}>{Icons[s.icon] || Icons.pizza}</span>
                        </div>
                        <div>
...
function ServiciosFeatured({ d, primary, primaryRgb, fontTitulo, fontCuerpo, BG, BG2, TEXT, MUTED, BORDER, isDark }) {
    const [featured, ...rest] = d.servicios || [];
    return (
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: "1.5rem" }} className="grid-2">
            {featured && (
                <AnimatedSection>
                    <div className="service-card" style={{ height: "100%", background: BG2, border: `1px solid ${BORDER}`, borderRadius: "20px", overflow: "hidden" }}>
                        <div className="card-accent" style={{ height: "3px", background: primary, width: "100%", transition: "width 0.4s ease" }} />
                        <div style={{ padding: "3rem 2.5rem" }}>
                            <div style={{ width: 64, height: 64, background: `rgba(${primaryRgb},0.15)`, border: `1px solid rgba(${primaryRgb},0.4)`, display: "flex", alignItems: "center", justifyContent: "center", color: primary, marginBottom: "1.5rem", borderRadius: "16px" }}>
                                <span style={{ width: 32, height: 32 }}>{Icons[featured.icon] || Icons.pizza}</span>
                            </div>
                            <div style={{ fontSize: "0.7rem", fontFamily: `'${fontTitulo}', serif`, letterSpacing: "0.2em", textTransform: "uppercase", color: primary, marginBottom: "0.75rem", fontWeight: 600 }}>Estrella del menú</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                {rest.map((s, i) => (
                    <AnimatedSection key={i} delay={i * 100}>
                        <div className="service-card" style={{ background: BG2, border: `1px solid ${BORDER}`, borderRadius: "16px", overflow: "hidden" }}>
                            <div className="card-accent" style={{ height: "3px", background: primary, width: 0, transition: "width 0.4s ease" }} />
                            <div style={{ padding: "1.5rem", display: "flex", gap: "1rem", alignItems: "flex-start" }}>
                                <div style={{ width: 40, height: 40, minWidth: 40, background: `rgba(${primaryRgb},0.1)`, display: "flex", alignItems: "center", justifyContent: "center", color: primary, borderRadius: "10px" }}>
                                    <span style={{ width: 20, height: 20 }}>{Icons[s.icon] || Icons.pizza}</span>
                                </div>
                                <div>
...
function NosotrosImage({ d, BG, BG2, TEXT, MUTED, BORDER, isDark }) {
    return (
        <div style={{ position: "relative", borderRadius: "24px", overflow: "hidden", aspectRatio: "4/3", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}>
            <img
                src={d.imagen_nosotros || 'https://images.unsplash.com/photo-1551632786-de41ec24a8f6?w=800&q=80'}
...
function NosotrosStats({ d, primary, fontTitulo, fontCuerpo, BG, BG2, TEXT, MUTED, BORDER, isDark }) {
    return (
...
export default function LandingPizzeria({ data, onEnter }) {
    const d = data || DEFAULT_DATA;
    const [scrolled, setScrolled] = useState(false);
    const [menuOpen, setMenuOpen] = useState(false);

    const isDark = (d.tema || 'dark') === 'dark';
    const primary = d.color_primario || '#d97706';
    const secondary = d.color_secundario || '#b45309';
    ...
    const iniciales = (d.nombre || "PZ").split(" ").slice(0, 2).map(w => w[0]).join("").toUpperCase();
        ...
        <div style={{ fontFamily: `'${fontCuerpo}', sans-serif`, background: BG, color: TEXT, overflowX: "hidden" }}>
            <style>{`
                ...
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .btn-outline { background: transparent; color: var(--text); border: 1.5px solid var(--border); cursor: pointer; font-family: '${fontTitulo}', serif; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; transition: all 0.2s; }
                .btn-outline:hover { border-color: var(--primary); color: var(--primary); }
                .service-card:hover { border-color: var(--primary); transform: translateY(-4px); box-shadow: 0 16px 40px rgba(0,0,0,0.1); }
                .service-card:hover .card-accent { width: 100%; }
                .testim-card { background: var(--bg2); border: 1px solid var(--border); border-radius: 20px; padding: 2rem; transition: border-color 0.3s; }
            ...
            <nav style={{
                ...
                <div className="desktop-nav" style={{ display: "flex", alignItems: "center", gap: "2.5rem" }}>
                    ...
                    <button className="btn-primary" style={{ padding: "0.6rem 1.4rem", fontSize: "0.85rem", borderRadius: "8px" }} onClick={() => scrollTo("contacto")}>Pedir Ahora</button>
                </div>
            ...
            {menuOpen && (
                <div style={{ position: "fixed", top: 65, left: 0, right: 0, zIndex: 999, background: BG, borderBottom: `1px solid ${BORDER}`, padding: "1.5rem 2rem", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                    ...
                    <button className="btn-primary" style={{ padding: "0.8rem", fontSize: "0.9rem", borderRadius: "8px" }} onClick={() => scrollTo("contacto")}>Pedir Ahora</button>
                </div>
            )}
            ...
            <section style={{ background: primary, padding: "2rem" }}>
                <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", maxWidth: 900, margin: "0 auto", textAlign: "center", gap: "1rem" }}>
                    {[
                        { n: d.anos_experiencia, l: "Años de tradición" },
                        { n: d.clientes_atendidos, l: "Pizzas servidas" },
                        { n: "100%", l: "Fresco diariamente" },
                        { n: "24/7", l: "Para ti" },
                    ].map((s, i) => (
                        <div key={i}>
            ...
            <section id="servicios" style={{ background: BG2, padding: "6rem 2rem" }}>
                <div style={{ maxWidth: 1200, margin: "0 auto" }}>
                    <AnimatedSection>
                        ...
                        <p style={{ textAlign: "center", fontFamily: `'${fontTitulo}', serif`, fontWeight: 600, fontSize: "0.8rem", letterSpacing: "0.2em", textTransform: "uppercase", color: primary, marginBottom: "0.5rem" }}>Nuestras ofertas</p>
                        <h2 style={{ fontFamily: `'${fontTitulo}', serif`, textAlign: "center", fontSize: "clamp(2rem,5vw,3rem)", fontWeight: 800, textTransform: "uppercase", letterSpacing: "-0.01em", marginBottom: "3.5rem", color: TEXT }}>Menú Especial</h2>
                    </AnimatedSection>
                    {(() => {
                        const props = { d, primary, primaryRgb, fontTitulo, fontCuerpo, BG, BG2, TEXT, MUTED, BORDER, isDark };
            ...
            <section id="nosotros" style={{ background: BG, padding: "6rem 2rem" }}>
                <div style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "center" }} className="grid-2">
                    <AnimatedSection>
                        <div className="divider"><div className="divider-line" /><div className="divider-dot" /><div className="divider-line" /></div>
                        <p style={{ fontFamily: `'${fontTitulo}', serif`, fontWeight: 600, fontSize: "0.8rem", letterSpacing: "0.2em", textTransform: "uppercase", color: primary, marginBottom: "0.5rem" }}>Nuestro compromiso</p>
                        <h2 style={{ fontFamily: `'${fontTitulo}', serif`, fontSize: "clamp(2rem,4vw,2.8rem)", fontWeight: 800, textTransform: "uppercase", lineHeight: 1.1, marginBottom: "1.5rem", color: TEXT }}>Hecha con pasión y amor</h2>
                        <p style={{ color: MUTED, lineHeight: 1.7, marginBottom: "2rem" }}>{d.descripcion}</p>
                        <button className="btn-primary" style={{ padding: "0.9rem 2rem", fontSize: "0.9rem", borderRadius: "12px" }} onClick={() => scrollTo("contacto")}>Conocer más</button>
                    </AnimatedSection>
            ...
            <section id="testimonios" style={{ background: BG2, padding: "6rem 2rem" }}>
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    <AnimatedSection>
                        <div className="divider"><div className="divider-line" /><div className="divider-dot" /><div className="divider-line" /></div>
                        <p style={{ textAlign: "center", fontFamily: `'${fontTitulo}', serif`, fontWeight: 600, fontSize: "0.8rem", letterSpacing: "0.2em", textTransform: "uppercase", color: primary, marginBottom: "0.5rem" }}>Opiniones</p>
                            ...
                            <AnimatedSection key={i} delay={i * 120}>
                                <div className="testim-card">
                                    ...
                                    <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                                        <div style={{ width: 48, height: 48, background: primary, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: `'${fontTitulo}', serif`, fontWeight: 800, fontSize: "1.1rem", color: "#fff", borderRadius: "12px" }}>
            ...
            <section id="contacto" style={{ background: BG, padding: "6rem 2rem" }}>
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    <AnimatedSection>
                        ...
                        <h2 style={{ fontFamily: `'${fontTitulo}', serif`, textAlign: "center", fontSize: "clamp(2rem,5vw,3rem)", fontWeight: 800, textTransform: "uppercase", marginBottom: "3.5rem", color: TEXT }}>¿Listo para disfrutar?</h2>
                    </AnimatedSection>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                                {[
                                    ...
                                    <div key={i} style={{ display: "flex", gap: "1rem", alignItems: "flex-start", padding: "1.25rem", background: BG2, border: `1px solid ${BORDER}`, borderRadius: "16px" }}>
                                        <div style={{ width: 42, height: 42, minWidth: 42, background: `rgba(${primaryRgb},0.12)`, border: `1px solid rgba(${primaryRgb},0.3)`, display: "flex", alignItems: "center", justifyContent: "center", color: primary, borderRadius: "10px" }}>
                        ...
                        <AnimatedSection delay={150}>
                            <div style={{ background: BG2, border: `1px solid ${BORDER}`, padding: "2.5rem", borderRadius: "20px" }}>
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                                    ...
                                    <input type="text" placeholder="¿Qué deseas ordenar?" style={{ borderRadius: "12px" }} />
                                    <textarea placeholder="Cuéntanos más de tu pedido..." style={{ borderRadius: "12px" }} />
                                    <button className="btn-primary" style={{ padding: "1rem", fontSize: "1rem", borderRadius: "12px", width: "100%" }}>Enviar Pedido</button>
                                </div>
                            </div>
                        </AnimatedSection>
                    </div>
            ...
            <footer style={{ background: BG2, borderTop: `1px solid ${BORDER}`, padding: "3rem 2rem 1.5rem" }}>
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "2rem", paddingBottom: "2rem", borderBottom: `1px solid ${BORDER}`, marginBottom: "1.5rem" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                            <div style={{ width: 36, height: 36, background: primary, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: `'${fontTitulo}', serif`, fontWeight: 800, color: "#fff", borderRadius: "6px" }}>{iniciales}</div>
            ...
            {onEnter && (
                <button onClick={onEnter} style={{
                    ...
                    onMouseOver={e => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = `0 8px 24px rgba(${primaryRgb},0.5)`; }}
                    onMouseOut={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = `0 4px 20px rgba(${primaryRgb},0.4)`; }}
                >
                    Entrar →
                </button>
            )}
